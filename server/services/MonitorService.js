    ...
    try {
      ...
      if (isNoHandlerError(err)) {
        if (this.logger && typeof this.logger.warn === 'function') {
          this.logger.warn(
            '[alertsForMonitorsV2] v2 alerts endpoint not available on target cluster'
          );
        } else {
          // eslint-disable-next-line no-console
          console.warn('[alertsForMonitorsV2] v2 alerts endpoint not available on target cluster');
        }
        return res.ok({
          body: { ok: false, resp: 'alerts v2 endpoint not available on the selected data source' },
        });
      }
