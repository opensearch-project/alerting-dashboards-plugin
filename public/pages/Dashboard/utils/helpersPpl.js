...
import { bucketColumns } from './tableUtilsPpl';
...
export function groupAlertsByTrigger(alerts, groupByMonitor = true) {
  ...
  const alertsByTriggers = new Map();
  ...
  return Array.from(alertsByTriggers, ([compositeKey, groupedAlerts]) => {
    const triggerID = groupByMonitor ? compositeKey.split('::')[0] : compositeKey;
    return { ...groupedAlerts, triggerID };
  });
}
...
export function addFirstAlert(firstAlert) {
  ...
  const newAlertList = _.cloneDeep(EMPTY_ALERT_LIST);
  newAlertList[state]++;
  newAlertList.total++;
...
export function insertGroupByColumn(groupBy = []) {
  const result = _.cloneDeep(bucketColumns);
  groupBy.forEach((fieldName) =>
    result.push({
...
export function findLongestStringField(pplRes) {
  if (
    !pplRes ||
    !pplRes.body ||
    !Array.isArray(pplRes.body.schema) ||
    !Array.isArray(pplRes.body.datarows)
  ) {
    return '';
  }
