...
const TriggerExpressionsPpl = ({ label, keyFieldName, valueFieldName, flyoutMode }) => (
  <EuiCompressedFormRow
    label={label}
      ...
      <EuiFlexItem grow={1}>
        <Field name={keyFieldName}>
          {({ field, form, meta }) => {
            ...
            return (
              <EuiCompressedFormRow isInvalid={!!(meta.touched && meta.error)} error={meta.error}>
                <EuiCompressedSelect
                  name={field.name}
                  value={safeValue}
...
export default TriggerExpressionsPpl;
