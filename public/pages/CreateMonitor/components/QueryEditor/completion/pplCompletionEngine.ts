...
import {
  COMMANDS, CLAUSE, LOGICAL, BOOL_FUNCS, OPERATORS,
  CONVERTED_TYPES, INTERVAL_UNITS, SPAN_UNITS,
  AGGS, MATH_FUNCS, DATE_FUNCS, TEXT_FUNCS,
  RELEVANCE_FUNCS, RELEVANCE_ARGS, FIELD_KEYWORDS,
  DATASET_TYPES, KMEANS_ARGS, AD_ARGS, CMD_ARGS_MISC,
  DEFAULT_FIELDS,
} from './pplDictionary';
...
function asKind(monaco: any, label: string): number {
  ...
  if (COMMANDS.includes(U) || CLAUSE.includes(U) || LOGICAL.includes(U) || DATASET_TYPES.includes(U)) {
    return monaco.languages.CompletionItemKind.Keyword;
  }
  if (AGGS.includes(U) || MATH_FUNCS.includes(U) || DATE_FUNCS.includes(U) || TEXT_FUNCS.includes(U) || RELEVANCE_FUNCS.includes(U)) {
...
export function buildSuggestions({
  ...
  const SNIPPETS: Item[] = [
    ...
    snip('STATS PERCENTILE', 'STATS PERCENTILE<${1:95}>(${2:field}) BY ${3:field}', 'Percentile agg'),
    ...
    snip('SIMPLE_QUERY_STRING', 'SIMPLE_QUERY_STRING([${1:field}^${2:1}], ${3:"query"}, ${4:OPERATOR}=${5:AND})', 'Relevance (multi-field)'),
  ];
  ...
  switch (scope) {
    ...
    case 'AfterPipe':
      pool = uniq([
        ...COMMANDS, ...CLAUSE, 'DESC', 'DATASOURCES', // SHOW helpers
      ]);
      extra = SNIPPETS.filter(s => ['SEARCH SOURCE =','SEARCH INDEX =','HEAD N','TOP N BY'].some(x => s.label.startsWith(x)));
      break;
    ...
    case 'AfterFromOrSourceOrIndex':
      ...
      extra = SNIPPETS.filter(s => s.label.startsWith('WHERE'))
        .concat(SNIPPETS.filter(s => s.label === 'BY SPAN()'));
      break;
    ...
    case 'AfterWhere':
      ...
      pool = uniq([
        ...FIELDS,
        ...MATH_FUNCS, ...DATE_FUNCS, ...TEXT_FUNCS, ...RELEVANCE_FUNCS,
        ...BOOL_FUNCS,
        ...CONVERTED_TYPES,
      ]);
    ...
    case 'AfterFields':
      // Field projections: allow +/- and wildcards, but strictly prefix filtered later
      pool = uniq(['+','-','*', ...FIELDS]);
      break;
    ...
    default:
      pool = uniq([
        ...COMMANDS, ...CLAUSE, ...LOGICAL,
        ...AGGS, ...MATH_FUNCS, ...DATE_FUNCS, ...TEXT_FUNCS,
        ...RELEVANCE_FUNCS, ...RELEVANCE_ARGS,
        ...CONVERTED_TYPES, ...INTERVAL_UNITS, ...SPAN_UNITS,
        ...FIELDS, ...DATASET_TYPES, ...KMEANS_ARGS, ...AD_ARGS, ...CMD_ARGS_MISC,
        ...extraKeywords,
      ]);
      break;
  }
  ...
  const filteredSnips  = SNIPPETS
    .concat(extra)
    .filter(s => startsWithInsensitive(s.label, prefix));
