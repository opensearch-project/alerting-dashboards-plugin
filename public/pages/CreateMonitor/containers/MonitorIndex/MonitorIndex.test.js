...
describe('MonitorIndex', () => {
  ...
  test('returns empty data stream array for cross-cluster patterns', async () => {
    const wrapper = getMountWrapper();
    expect(
      await wrapper.find(MonitorIndex).instance().handleQueryDataStreams('remote:logs')
    ).toEqual([]);
  });

  test('returns empty data stream array when data.ok is false', async () => {
    httpClientMock.post.mockResolvedValue({ ok: false });
    const wrapper = getMountWrapper();
    expect(await wrapper.find(MonitorIndex).instance().handleQueryDataStreams('repro')).toEqual([]);
  });

  test('returns empty data stream array when the request throws', async () => {
