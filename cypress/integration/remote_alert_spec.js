...
import sampleQueryLevelRemoteMonitorWorkflow from '../fixtures/sample_query_level_remote_monitor_workflow';

const TESTING_INDEX = 'alerting_test';

if (Cypress.env('remote_cluster_enabled')) {
  describe('Remote alerts', () => {
    beforeEach(() => {
    ...
    describe("can be in 'Active' state", () => {
      before(() => {
        ...
        sampleQueryLevelRemoteMonitorWithAlwaysTrueTrigger.name += `-${Cypress.config(
          'unique_number'
        )}`;
    ...
    describe("can be in 'Acknowledged' state", () => {
      before(() => {
        ...
        cy.createAndExecuteMonitor(sampleQueryLevelRemoteMonitorWithAlwaysTrueTrigger);
      });
    ...
    describe("can be in 'Completed' state", () => {
      before(() => {
        ...
        cy.deleteRemoteIndexByName('alerting*');
        Cypress.config('unique_number', `${Date.now()}`);
        // Modify the monitor name to be unique
        sampleQueryLevelRemoteMonitorWorkflow.name += `-${Cypress.config('unique_number')}`;
        cy.createAndExecuteMonitor(sampleQueryLevelRemoteMonitorWorkflow);
      });
      ...
      it('when the trigger condition is not met after met once', () => {
        ...
        // Create an index
        cy.createRemoteIndexByName(TESTING_INDEX);

        // Insert a document
        cy.insertDocumentToRemoteIndex(TESTING_INDEX, 1, {});
      ...
      after(() => {
        ...
        cy.deleteRemoteIndexByName(TESTING_INDEX);
      });
    });
    ...
    describe("can be in 'Error' state", () => {
      before(() => {
        ...
        sampleQueryLevelRemoteMonitorWithAlwaysTrueTrigger.triggers[0].actions = [
